import afterpay from './afterpay.svg';
import airtelMoney from './airtel_money.svg';
import alipay from './alipay.svg';
import amazon from './amazon.svg';
import ambank from './ambank.svg';
import americanExpress from './american_express.svg';
import amex from './amex.svg';
import applePay from './apple_pay.svg';
import arhaus from './arhaus.svg';
import arvato from './arvato.svg';
import atmbersama from './atmbersama.svg';
import bancnet from './bancnet.svg';
import bancontact from './bancontact.svg';
import bitcoin from './bitcoin.svg';
import bitcoinCash from './bitcoin_cash.svg';
import bogus from './bogus.svg';
import boleto from './boleto.svg';
import cash from './cash.svg';
import cimb from './cimb.svg';
import circlek from './circlek.svg';
import citadele from './citadele.svg';
import clearpay from './clearpay.svg';
import collectorBank from './collector_bank.svg';
import dai from './dai.svg';
import dailyyamazaki from './dailyyamazaki.svg';
import dankort from './dankort.svg';
import danskeBank from './danske_bank.svg';
import dash from './dash.svg';
import dinersClub from './diners_club.svg';
import discover from './discover.svg';
import dnb from './dnb.svg';
import dogecoin from './dogecoin.svg';
import dwolla from './dwolla.svg';
import ebucks from './ebucks.svg';
import eftSecure from './eft_secure.svg';
import eghl from './eghl.svg';
import elo from './elo.svg';
import elv from './elv.svg';
import enets from './enets.svg';
import eos from './eos.svg';
import epayments from './epayments.svg';
import eps from './eps.svg';
import esrPaymentslipSwitzerland from './esr_paymentslip_switzerland.svg';
import ethereum from './ethereum.svg';
import familymart from './familymart.svg';
import forbrugsforeningen from './forbrugsforeningen.svg';
import fpx from './fpx.svg';
import freecharge from './freecharge.svg';
import generic from './generic.svg';
import giftCard from './gift-card.svg';
import giropay from './giropay.svg';
import googlePay from './google_pay.svg';
import googleWallet from './google_wallet.svg';
import grabpay from './grabpay.svg';
import hongleongbank from './hongleongbank.svg';
import hyper from './hyper.svg';
import hypercard from './hypercard.svg';
import ideal from './ideal.svg';
import in3 from './in3.svg';
import interac from './interac.svg';
import jcb from './jcb.svg';
import jousto from './jousto.svg';
import klarnaPayLater from './klarna-pay-later.svg';
import klarnaPayNow from './klarna-pay-now.svg';
import klarnaSliceIt from './klarna-slice-it.svg';
import klarna from './klarna.svg';
import krediidipank from './krediidipank.svg';
import krungsri from './krungsri.svg';
import laser from './laser.svg';
import lawson from './lawson.svg';
import laybuy from './laybuy.svg';
import lhv from './lhv.svg';
import litecoin from './litecoin.svg';
import maestro from './maestro.svg';
import mash from './mash.svg';
import master from './master.svg';
import mastercard from './mastercard.svg';
import masterpass from './masterpass.svg';
import maybank from './maybank.svg';
import ministop from './ministop.svg';
import mobicred from './mobicred.svg';
import mobikwik from './mobikwik.svg';
import mobilepay from './mobilepay.svg';
import mondido from './mondido.svg';
import monero from './monero.svg';
import mpesa from './mpesa.svg';
import netbanking from './netbanking.svg';
import nordea from './nordea.svg';
import olaMoney from './ola_money.svg';
import op from './op.svg';
import ovo from './ovo.svg';
import ozow from './ozow.svg';
import pagoefectivo from './pagoefectivo.svg';
import payd from './payd.svg';
import payfastInstantEft from './payfast_instant_eft.svg';
import paymarkOnlineEftpos from './paymark_online_eftpos.svg';
import paypal from './paypal.svg';
import paysafecard from './paysafecard.svg';
import paysera from './paysera.svg';
import paytm from './paytm.svg';
import payzapp from './payzapp.svg';
import pivo from './pivo.svg';
import prepaysolutions from './prepaysolutions.svg';
import przelew24 from './przelew24.svg';
import publicbank from './publicbank.svg';
import qrph from './qrph.svg';
import ratepay from './ratepay.svg';
import rhbbank from './rhbbank.svg';
import rupay from './rupay.svg';
import santander from './santander.svg';
import satispay from './satispay.svg';
import seb from './seb.svg';
import seveneleven from './seveneleven.svg';
import sezzle from './sezzle.svg';
import shopPay from './shop_pay.svg';
import siamcommercial from './siamcommercial.svg';
import sofort from './sofort.svg';
import spraypay from './spraypay.svg';
import sunkus from './sunkus.svg';
import swedbank from './swedbank.svg';
import swish from './swish.svg';
import trustly from './trustly.svg';
import twint from './twint.svg';
import ubp from './ubp.svg';
import unionpay from './unionpay.svg';
import unipay from './unipay.svg';
import uob from './uob.svg';
import usdc from './usdc.svg';
import vPay from './v_pay.svg';
import venmo from './venmo.svg';
import viabill from './viabill.svg';
import vipps from './vipps.svg';
import visa from './visa.svg';
import visaelectron from './visaelectron.svg';
import wechatpay from './wechatpay.svg';
import ymobile from './ymobile.svg';
import qris from './qris.svg';
import sam from './sam.svg';
import axs from './axs.svg';

export const CARD_IMAGE_BY_BRAND = new Map([
  ['afterpay', afterpay],
  ['airtel_money', airtelMoney],
  ['alipay', alipay],
  ['amazon', amazon],
  ['ambank', ambank],
  ['american_express', americanExpress],
  ['amex', amex],
  ['apple_pay', applePay],
  ['arhaus', arhaus],
  ['arvato', arvato],
  ['atmbersama', atmbersama],
  ['bancnet', bancnet],
  ['bancontact', bancontact],
  ['bitcoin', bitcoin],
  ['bitcoin_cash', bitcoinCash],
  ['bogus', bogus],
  ['boleto', boleto],
  ['cash', cash],
  ['cimb', cimb],
  ['circlek', circlek],
  ['citadele', citadele],
  ['clearpay', clearpay],
  ['collector_bank', collectorBank],
  ['dai', dai],
  ['dailyyamazaki', dailyyamazaki],
  ['dankort', dankort],
  ['danske_bank', danskeBank],
  ['dash', dash],
  ['diners_club', dinersClub],
  ['discover', discover],
  ['dnb', dnb],
  ['dogecoin', dogecoin],
  ['dwolla', dwolla],
  ['ebucks', ebucks],
  ['eft_secure', eftSecure],
  ['eghl', eghl],
  ['elo', elo],
  ['elv', elv],
  ['enets', enets],
  ['eos', eos],
  ['epayments', epayments],
  ['eps', eps],
  ['esr_paymentslip_switzerland', esrPaymentslipSwitzerland],
  ['ethereum', ethereum],
  ['familymart', familymart],
  ['forbrugsforeningen', forbrugsforeningen],
  ['fpx', fpx],
  ['freecharge', freecharge],
  ['generic', generic],
  ['gift_card', giftCard],
  ['giropay', giropay],
  ['google_pay', googlePay],
  ['google_wallet', googleWallet],
  ['grabpay', grabpay],
  ['hongleongbank', hongleongbank],
  ['hyper', hyper],
  ['hypercard', hypercard],
  ['ideal', ideal],
  ['in3', in3],
  ['interac', interac],
  ['jcb', jcb],
  ['jousto', jousto],
  ['klarna_pay_later', klarnaPayLater],
  ['klarna_pay_now', klarnaPayNow],
  ['klarna_slice_it', klarnaSliceIt],
  ['klarna', klarna],
  ['krediidipank', krediidipank],
  ['krungsri', krungsri],
  ['laser', laser],
  ['lawson', lawson],
  ['laybuy', laybuy],
  ['lhv', lhv],
  ['litecoin', litecoin],
  ['maestro', maestro],
  ['mash', mash],
  ['master', master],
  ['mastercard', mastercard],
  ['masterpass', masterpass],
  ['maybank', maybank],
  ['ministop', ministop],
  ['mobicred', mobicred],
  ['mobikwik', mobikwik],
  ['mobilepay', mobilepay],
  ['mondido', mondido],
  ['monero', monero],
  ['mpesa', mpesa],
  ['netbanking', netbanking],
  ['nordea', nordea],
  ['ola_money', olaMoney],
  ['op', op],
  ['ovo', ovo],
  ['ozow', ozow],
  ['pagoefectivo', pagoefectivo],
  ['payd', payd],
  ['payfast_instant_eft', payfastInstantEft],
  ['paymark_online_eftpos', paymarkOnlineEftpos],
  ['paypal', paypal],
  ['paysafecard', paysafecard],
  ['paysera', paysera],
  ['paytm', paytm],
  ['payzapp', payzapp],
  ['pivo', pivo],
  ['prepaysolutions', prepaysolutions],
  ['przelew24', przelew24],
  ['publicbank', publicbank],
  ['qrph', qrph],
  ['ratepay', ratepay],
  ['rhbbank', rhbbank],
  ['rupay', rupay],
  ['santander', santander],
  ['satispay', satispay],
  ['seb', seb],
  ['seveneleven', seveneleven],
  ['sezzle', sezzle],
  ['shop_pay', shopPay],
  ['siamcommercial', siamcommercial],
  ['sofort', sofort],
  ['spraypay', spraypay],
  ['sunkus', sunkus],
  ['swedbank', swedbank],
  ['swish', swish],
  ['trustly', trustly],
  ['twint', twint],
  ['ubp', ubp],
  ['unionpay', unionpay],
  ['unipay', unipay],
  ['uob', uob],
  ['usdc', usdc],
  ['v_pay', vPay],
  ['venmo', venmo],
  ['viabill', viabill],
  ['vipps', vipps],
  ['visa', visa],
  ['visaelectron', visaelectron],
  ['wechatpay', wechatpay],
  ['ymobile', ymobile],
  ['qris', qris],
  ['sam', sam],
  ['axs', axs],
]);
